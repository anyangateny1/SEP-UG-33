...
void BlockModel::read_model() {
  ...
  for (int z = 0; z < z_count; ++z) {
    ...
    // Skip blank separator between slices (Python called input()
    // unconditionally)
    if (z < z_count - 1) {
      string sep;
      getline_strict(sep);
      // We don't enforce emptiness; just consume one line to replicate Python
      // behavior.
    }
  }
...
void BlockModel::getline_strict(string& out) {
  if (!std::getline(std::cin, out))
    out.clear(); // On EOF, act like Python input(): returns ""
  if (!out.empty() && out.back() == '\r')
    out.pop_back(); // Normalize CRLF
}
  ...
  for (char c : line) {
    if (c == ',') {
      if (!cur.empty()) {
        ...
        vals.push_back(0);
      } // tolerate empty segment (unlikely here)
...
Vec3<char> BlockModel::slice_model(const Vec3<char>& src, int depth, int y0,
                                   int y1, int x0, int x1) {
  Vec3<char> out(depth,
                 vector<vector<char>>(y1 - y0, vector<char>(x1 - x0, '\0')));
...
void BlockModel::compress_slices(int top_slice, int n_slices) {
  for (int y = 0; y < y_count; y += parent_y) {
    for (int x = 0; x < x_count; x += parent_x) {
      ...
      Vec3<char> model_slices =
          slice_model(model, depth, y, parentBlock.y_end, x, parentBlock.x_end);
