...
BlockGrowth::BlockGrowth(const Flat3D<char>& model_slices, const unordered_map<char, string>& tag_table)
...
bool BlockGrowth::all_compressed() const {
    ...
    if (total_size >= 64) {
        ...
        for (; processed < total_size; ++processed) {
            if (compressed.data[processed] == 0)
                return false;
        }
    } else {
...
Block BlockGrowth::find_best_rectangular_block(const Block& parent_block) {
    ...
    std::sort(all_candidates.begin(), all_candidates.end(), [](const Block& a, const Block& b) {
...
Block BlockGrowth::find_largest_contiguous_block(const Block& parent_block) {
    ...
    mark_compressed(
        best_block.z_offset, best_block.z_offset + best_block.depth, best_block.y_offset,
        best_block.y_offset + best_block.height, best_block.x_offset, best_block.x_offset + best_block.width, 1
    );
...
Block BlockGrowth::grow_largest_block_from_position(int start_x, int start_y, int start_z, char tag) {
    ...
    for (int x = start_x + 1; x < parent_block.width; ++x) {
        if (compressed.at(start_z, start_y, x) != 0 || model.at(start_z, start_y, x) != tag)
            break;
        max_width++;
    }
    ...
    for (int y = start_y + 1; y < parent_block.height; ++y) {
        if (compressed.at(start_z, y, start_x) != 0 || model.at(start_z, y, start_x) != tag)
            break;
        max_height++;
    }
    ...
    for (int z = start_z + 1; z < parent_block.depth; ++z) {
        if (compressed.at(z, start_y, start_x) != 0 || model.at(z, start_y, start_x) != tag)
            break;
        max_depth++;
    }
            ...
            for (int d = 1; d <= max_depth; ++d) {
                ...
                if (valid && (w * h * d) > best_candidate.volume) {
                    best_candidate = Block(
                        parent_block.x + start_x, parent_block.y + start_y, parent_block.z + start_z, w, h, d, tag,
...
bool BlockGrowth::window_is_all_uncompressed(int z0, int z1, int y0, int y1, int x0, int x1) const {
    ...
    for (int z = z0; z < z1; ++z) {
        for (int y = y0; y < y1; ++y) {
            ...
            for (; x <= x1 - 4; x += 4) {
                if (compressed.at(z, y, x) != 0 || compressed.at(z, y, x + 1) != 0 || compressed.at(z, y, x + 2) != 0 ||
                    compressed.at(z, y, x + 3) != 0) {
                    return false;
                }
            }
...
void BlockGrowth::grow_block(Block& current, Block& best_block) {
    ...
    if (can_grow_x && can_grow_y) {
        ...
        for (int zz = z; zz < z_end && ok; ++zz) {
            // Check corner
            if (model.at(zz, y_end, x_end) != b.tag)
                ok = false;
            if (compressed.at(zz, y_end, x_end) != 0)
                ok = false;
            // Check new edges
            for (int xx = x; xx < x_end && ok; ++xx) {
                if (model.at(zz, y_end, xx) != b.tag)
                    ok = false;
                if (compressed.at(zz, y_end, xx) != 0)
                    ok = false;
            }
            for (int yy = y; yy < y_end && ok; ++yy) {
                if (model.at(zz, yy, x_end) != b.tag)
                    ok = false;
                if (compressed.at(zz, yy, x_end) != 0)
                    ok = false;
            }
        }
        if (ok) {
    ...
    std::sort(options, options + 3, [](const GrowthOption& a, const GrowthOption& b) { return a.gain > b.gain; });

    for (int i = 0; i < 3; ++i) {
        if (options[i].gain == 0)
            break;  // No more valid directions
        ...
        if (direction == 0) {  // +X growth
            ...
            for (int yy = y; yy < y_end && ok; ++yy)
                for (int xx = x; xx < x_end && ok; ++xx) {
                    if (model.at(z_end, yy, xx) != b.tag)
                        ok = false;
                    if (compressed.at(z_end, yy, xx) != 0)
                        ok = false;
                }
            if (ok) {
...
std::string BlockGrowth::run_to_string(Block parent_block_) {
    ...
    while (!all_compressed()) {
        ...
        result << largest_block.x << "," << largest_block.y << "," << largest_block.z << "," << largest_block.width
               << "," << largest_block.height << "," << largest_block.depth << "," << label << "\n";
    }
