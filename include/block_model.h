...
private:
  ...
  static Flat3D<char> slice_model(const Flat3D<char>& src, int depth, int y0,
                                  int y1, int x0, int x1);

  void compress_slices(int top_slice, int n_slices);
  void process_parent_block(int x, int y, int z, int width, int height,
                            int depth, char tag, int n_slices);
  std::string process_parent_block_to_string(int x, int y, int z, int width,
                                             int height, int depth, char tag,
                                             int n_slices);
  std::string
  process_parent_block_to_string_safe(const Flat3D<char>& model_slices,
