...
// BlockGrowth encapsulates the "fit & grow" compression logic for a parent
// block over a sub-volume (model_slices). The tag_table maps single-char tags
// to labels.
class BlockGrowth {
public:
...
private:
  ...
  bool window_is_all(char val, int z0, int z1, int y0, int y1, int x0,
                     int x1) const;
  bool window_is_all_uncompressed(int z0, int z1, int y0, int y1, int x0,
                                  int x1) const;
